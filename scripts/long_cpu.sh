...
ALGO=${ALGO:-sac}
FLOORPLAN=${FLOORPLAN:-office_4room}
TOTAL_TIMESTEPS=${TOTAL_TIMESTEPS:-}
RESUME_FROM=${RESUME_FROM:-}
RESUME_FROM=${RESUME_FROM/#\~/$HOME}   # expand ~ since sbatch --export doesn't do tilde expansion
...
~/.conda/envs/sbsim/bin/python scripts/train_rl.py \
    ...
    ${RESUME_WANDB_ID:+--resume_wandb_id ${RESUME_WANDB_ID}} \
